<script lang="ts">
	...
	// Form data - initialize with existing income source data
	let formData: IncomeSourceFormData = {
		name: incomeSource.name,
		amount: incomeSource.amount,
		frequency: incomeSource.frequency,
		custom_frequency_days: incomeSource.custom_frequency_days || 30,
		description: incomeSource.description || '',
		is_active: incomeSource.is_active
	}
	...
	// Load existing income source names for uniqueness validation (excluding current one)
	async function loadExistingIncomeSourceNames() {
		...
		try {
			const { data, error } = await supabase
				...
				.select('name')
				.eq('user_id', $user.id)
				.neq('id', incomeSource.id) // Exclude current income source
	...
	function validateForm(): boolean {
		...
		// Check name uniqueness (excluding current income source)
	...
	async function handleSubmit() {
		...
		if (!$user) {
			toastHelpers.error('You must be logged in to edit an income source')
			return
		}
		...
		try {
			...
			if (error) {
				...
				if (error.code === '23505') {
					...
					toastHelpers.error('Failed to update income source. Please try again.')
				}
				return
			}
			
			toastHelpers.success(`Income source "${sanitizedData.name}" updated successfully!`)
...
<div class="edit-income-source-form">
	<form on:submit|preventDefault={handleSubmit} class="space-y-6" novalidate>
		<!-- Form Header -->
		<div class="border-b border-gray-200 pb-4">
			<h3 class="text-lg font-medium text-gray-900">Edit Income Source</h3>
