<script lang="ts">
	...
	let minimumPayment: number | undefined = envelope.minimum_payment
	let loading = false
	...
	async function loadFormData() {
		...
		try {
			...
			// Load existing envelope names for validation (excluding current envelope)
	...
	function validateName(value: string): string[] {
		...
		// Check for duplicate names (case-insensitive, excluding current envelope)
	...
	async function handleSubmit() {
		if (!$user) {
			toastHelpers.error('You must be logged in to update an envelope')
			return
		}
		...
		try {
			...
			if (type === 'savings') {
				...
				// Clear debt-specific fields
				envelopeData.apr = null
				envelopeData.minimum_payment = null
			} else if (type === 'debt') {
				envelopeData.apr = apr
				envelopeData.minimum_payment = minimumPayment
				// Clear savings-specific fields
				envelopeData.target_amount = null
				envelopeData.target_date = null
			} else {
				// Regular envelope - clear all type-specific fields
				envelopeData.target_amount = null
				envelopeData.target_date = null
				envelopeData.apr = null
				envelopeData.minimum_payment = null
			...
			if (error) {
				...
				if (error.code === '23505') {
					...
					toastHelpers.error('Failed to update envelope. Please try again.')
				}
			} else {
				toastHelpers.success(`Envelope "${data.name}" updated successfully`)
...
<div class="edit-envelope-form">
	<form on:submit|preventDefault={handleSubmit} class="space-y-6" novalidate>
		<!-- Basic Information Section -->
		<div class="space-y-4">
			<div class="border-b border-gray-200 pb-4">
				<h3 class="text-lg font-medium text-gray-900">Edit Envelope</h3>
				<p class="text-sm text-gray-600 mt-1">Update the details for your envelope</p>
			</div>
		...
		{#if type === 'savings'}
			<div class="space-y-4">
				<div class="border-b border-gray-200 pb-4">
					...
					<p class="text-sm text-gray-600 mt-1">Update your savings target and timeline</p>
				</div>
		...
		{:else if type === 'debt'}
			<div class="space-y-4">
				<div class="border-b border-gray-200 pb-4">
					...
					<p class="text-sm text-gray-600 mt-1">Update debt details for tracking and payoff planning</p>
				</div>
		...
		<div class="space-y-4">
			<div class="border-b border-gray-200 pb-4">
				...
				<p class="text-sm text-gray-600 mt-1">See how your updated envelope will look</p>
			</div>
		...
		<div class="flex flex-col sm:flex-row gap-3 pt-6 border-t border-gray-200">
			<div class="flex-1 sm:flex-none">
				<LoadingButton
					...
					fullWidth={true}
				>
					{loading ? 'Updating Envelope...' : 'Update Envelope'}
				</LoadingButton>
			</div>
